...
class ToDoDialogFragment : DialogFragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        if (arguments != null){
            ...
            toDoData = ToDoData(arguments?.getString("taskId").toString() ,arguments?.getString("task").toString(), R.drawable.baseline_menu_24)
        ...
        binding.todoNextBtn.setOnClickListener {
            ...
            if (todoTask.isNotEmpty()) {
                if (toDoData == null) {
                    listener?.let { it.saveTask(todoTask, binding.todoEt) }
                } else {
                    toDoData!!.task = todoTask
                    listener?.let { it.updateTask(toDoData!!, binding.todoEt) }
                }
            }
        }
    }
